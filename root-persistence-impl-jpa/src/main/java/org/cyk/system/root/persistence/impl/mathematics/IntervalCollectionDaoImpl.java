...
public class IntervalCollectionDaoImpl extends AbstractCollectionDaoImpl<IntervalCollection,Interval> implements IntervalCollectionDao,Serializable {
	...
	protected void namedQueriesInitialisation() {
		...
		registerNamedQuery(readLowestValue, "SELECT MIN(interval.low.value) FROM Interval interval WHERE interval.collection = :collection",BigDecimal.class);
		registerNamedQuery(readHighestValue, "SELECT MAX(interval.high.value) FROM Interval interval WHERE interval.collection = :collection",BigDecimal.class);
	}
